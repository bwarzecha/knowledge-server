...
class Config:
    ...
    def __init__(self, env_file: Optional[str] = None):
        ...
        self.skip_hidden_files = (
            os.getenv("SKIP_HIDDEN_FILES", "true").lower() == "true"
        )
        self.supported_extensions = os.getenv(
            "SUPPORTED_EXTENSIONS", ".json,.yaml,.yml"
        ).split(",")
        self.log_processing_progress = (
            os.getenv("LOG_PROCESSING_PROGRESS", "true").lower() == "true"
        )

        # Markdown Processing
        self.markdown_source_dir = os.getenv(
            "MARKDOWN_SOURCE_DIR", self.openapi_specs_dir
        )
        self.markdown_max_tokens = int(os.getenv("MARKDOWN_MAX_TOKENS", "1000"))
        self.markdown_file_extensions = os.getenv(
        ...
        self.retrieval_max_primary_results = int(
            os.getenv("RETRIEVAL_MAX_PRIMARY_RESULTS", "5")
        )
        self.retrieval_max_total_chunks = int(
            os.getenv("RETRIEVAL_MAX_TOTAL_CHUNKS", "15")
        )
        ...
        self.context_prioritize_primary = (
            os.getenv("CONTEXT_PRIORITIZE_PRIMARY", "true").lower() == "true"
        )

        # Intelligent Chunk Filtering Configuration
        self.chunk_filtering_oversample_multiplier = float(
            os.getenv("CHUNK_FILTERING_OVERSAMPLE_MULTIPLIER", "1.6")
        )
        self.chunk_filtering_llm_timeout_ms = int(
            os.getenv("CHUNK_FILTERING_LLM_TIMEOUT_MS", "10000")
        )
        ...
        self.reranker_llm_model = os.getenv(
        ...
        self.reranker_llm_temperature = float(
            os.getenv("RERANKER_LLM_TEMPERATURE", "0.1")
        )
        ...
        self.research_agent_llm_max_tokens = int(
            os.getenv("RESEARCH_AGENT_LLM_MAX_TOKENS", "40000")
        )
        self.research_agent_llm_retry_max_attempts = int(
            os.getenv("RESEARCH_AGENT_LLM_RETRY_MAX_ATTEMPTS", "3")
        )

        # Chunk Filtering LLM Retry Configuration
        self.chunk_filtering_llm_retry_max_attempts = int(
            os.getenv("CHUNK_FILTERING_LLM_RETRY_MAX_ATTEMPTS", "3")
        )

        # Validation
        self.min_openapi_version = os.getenv("MIN_OPENAPI_VERSION", "3.0.0")
        self.require_info_section = (
            os.getenv("REQUIRE_INFO_SECTION", "true").lower() == "true"
        )
        self.require_paths_or_components = (
            os.getenv("REQUIRE_PATHS_OR_COMPONENTS", "true").lower() == "true"
        )
